...
extension URL {
	...
	/// determined.
	internal var typeIdentifier: Result<String, Error> {
		var error: NSError?
...
extension FileManager {
	...
	public func carthage_enumerator(at url: URL, includingPropertiesForKeys keys: [String], options: NSDirectoryEnumerationOptions = [], catchErrors: Bool = false) -> SignalProducer<(NSDirectoryEnumerator, URL), Error> {
	...
	internal func carthage_enumerator(at url: URL, includingPropertiesForKeys keys: [URLResourceKey]? = nil, options: NSDirectoryEnumerationOptions = [], catchErrors: Bool = false) -> SignalProducer<(NSDirectoryEnumerator, URL), Error> {
		return SignalProducer { observer, disposable in
			let enumerator = self.enumerator(at: url, includingPropertiesForKeys: keys, options: options) { (url, error) in
				if catchErrors {
					return true
				} else {
					observer.send(error: Error.readFailed(url, error))
					return false
				}
			}!
